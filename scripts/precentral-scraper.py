    ...
    def endElement(self,name):
        ...
        if ((name == "application") or (name == "theme")):
            ...
            if (m):
                ...
                if (name == "theme"):
                    id = "net.precentral.themes." + self.title.strip().lower().replace("&#58; ","-").replace("_quick_install_2.0","").replace("_","-").replace(": ","-").replace("&","").replace("#","").replace("  "," ").replace(" ","-").replace(".","").replace("'","").replace("(","").replace(")","").replace("+","").replace("!","").replace("/","").rstrip("-") + "_" + self.version + "_all"
...
#                    if (id == "net.precentral.themes.elf-at-night_1_all"): return
    ...
    def characters (self, ch): 
        if (self.getData) :
            self.data += ch.encode('utf-8').replace("'", "\\'").replace('"', '\\"').replace(': ', '&#58; ').replace('\r', '').replace('\n', '')
